...
class GarchCoreMixin:
    ...
    def european_option(self, normstrike, *, T=None, put=False):
        ...
        if put:
            def payoff(logp, k): return np.maximum(k-np.exp(logp), 0)
        else:
            def payoff(logp, k): return np.maximum(np.exp(logp)-k, 0)
            ...
            for (tau, quant) in enumerate(reversed(all_quantizations)):
                ...
                continuation = np.tensordot(
                    ...
                    axes=(
                        ...
                        # First ndim (all) dimensions
                        tuple(range(0, value.ndim))
                    )
                )
    ...
    def optimal_hedging_surface(self, strike, T=21, put=False):
        ...
        if put:
            def payoff(lp): return np.maximum(strike-np.exp(lp), 0)
        else:
            def payoff(lp): return np.maximum(np.exp(lp)-strike, 0)
        all_quantizations = self.quantizations[:T]
        # Initialization
        quant = all_quantizations[-1]
