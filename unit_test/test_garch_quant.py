...
class TestVoronoi(pymaat.testing.TestCase):
...
class TestQuantizerState(pymaat.testing.TestCase):
    pass

class TestFirstVarianceQuantizerState(pymaat.testing.TestCase):

    nquant = 4

    first_state = pymaat.garch.quant._FirstVarianceQuantizerState(
            nquant, first_variance)


class TestVarianceQuantizerState(pymaat.testing.TestCase):
    ...
    def test_zeroth_order_integral(self):
        expected_value = self.quantized_integral(
                ...
                self.state.grid)
        self.assert_almost_equal(self.state.integrals[0],
    ...
    def test_first_order_integral(self):
        expected_value = self.quantized_integral(
                ...
                self.state.grid)
        self.assert_almost_equal(self.state.integrals[1],
    ...
    def test_second_order_integral(self):
        expected_value = self.quantized_integral(
                ...
                self.state.grid)
        self.assert_almost_equal(self.state.integrals[2],
    ...
    def assert_integral_derivative(self, order=0, lag=0):
        ...
        for j in range(self.nquant-np.abs(lag)):
            ...
            self.assert_derivative_at(derivative,
                    func, self.state.grid[j+(lag==1)], rtol=1e-3)
    ...
    def test_distortion(self):
        self.assert_almost_equal(self.state.distortion,
                self.distortion(self.state.grid),
                msg='Incorrect distortion', rtol=1e-2)

    def test_distortion_gradient(self):
        self.assert_gradient_at(self.state.gradient,
                self.distortion,
                self.state.grid,
                rtol=1e-2)

    def test_distortion_hessian(self):
        self.assert_hessian_at(self.state.hessian,
                self.distortion,
                self.state.grid,
                rtol=1e-2, atol=1e-5)
    ...
    def assert_inv_trans_is_in_space_for(self, x):
        self.assert_true(np.diff(self.state.grid)>0, msg='is not sorted')
        # The first grid point could be equal to minimum variance
        # => test second point ...
        self.assert_true(self.state.grid[1]>self.state.h_min)
        # The first voronoi point must be strictly greater than minimum
        # variance
        self.assert_true(
                0.5*(self.state.grid[0]+self.state.grid[1])
                >self.state.h_min,
    ...
    def transform(self, x):
        state = pymaat.garch.quant._VarianceQuantizerState(model,
                self.nquant, self.prev_state, x)
        return state.grid

    def test_trans_jacobian(self):
        self.assert_jacobian_at(self.state.jacobian,
                self.transform,
    ...
    def test_distortion_from_x_gradient(self):
        self.assert_gradient_at(self.state.transformed_gradient,
             self.distortion_from_x, self.x, rtol=1e-2)

    def test_distortion_from_x_hessian(self):
        self.assert_hessian_at(self.state.transformed_hessian,
                self.distortion_from_x, self.x, rtol=1e-2)

    # Test transition probabilities

    def test_trans_proba_size(self):
        self.assert_equal(self.state.transition_probability.shape,
